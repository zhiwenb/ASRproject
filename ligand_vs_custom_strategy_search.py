#!/usr/bin/env python3
"""
Evaluate custom acquisition strategies for the course project.

New methods:
- hit_seeking: uncertainty + predicted positive probability
- diversity_hit_seeking: diversity-aware version of the same hybrid score
"""
...
def run_custom_strategy_search():
    data_dir = Path("data")
    output_dir = Path("results_course_project/custom_strategy")
    ensure_dir(output_dir)

    configs = [
        ("Uncertainty sampling", "uncertainty", {}),
        ("Diversity-aware uncertainty", "diversity", {}),
        ("Hit-seeking (alpha=0.50)", "hit_seeking", {"alpha": 0.50}),
        ("Hit-seeking (alpha=0.25)", "hit_seeking", {"alpha": 0.25}),
        ("Diversity-hit-seeking (alpha=0.50)", "diversity_hit_seeking", {"alpha": 0.50}),
        ("Diversity-hit-seeking (alpha=0.25)", "diversity_hit_seeking", {"alpha": 0.25}),
    ]

    rows = []
    for dataset_name in ["bace", "bbbp"]:
        x, y = load_dataset(data_dir, dataset_name)
        for label, query_name, extra in configs:
            print(f"Running {dataset_name.upper()} custom strategy: {label}")
    ...
    write_csv(rows, output_dir / "custom_strategy_results.csv")


if __name__ == "__main__":
    run_custom_strategy_search()
